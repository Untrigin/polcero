...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "construction_page" });
  return {
    title: t("meta_title"),
    description: t("meta_desc"),
    alternates: { canonical: `https://robotic4u.com/${locale}/solutions/construction` },
  };
}
...
const platforms = [
  ...
  { name: "Built Robotics Exosystem", note: "Autonomous excavator retrofit for trenching and grading." },
  { name: "Hilti Jaibot", note: "Semi-autonomous ceiling drilling for MEP installations." },
  { name: "POLCERO-T6", note: "Heavy tracked chassis for rough terrain, material transport." },
  { name: "POLCERO-Q4", note: "Quadruped for stair climbing, indoor inspection, irregular terrain." },
];

export default async function ConstructionPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations("construction_page");
  const base = `/${locale}`;
    ...
    <>
      <section className="py-16 px-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <SectionHeader badge={t("jobs_badge")} title={<>{t("jobs_title")}</>} />
